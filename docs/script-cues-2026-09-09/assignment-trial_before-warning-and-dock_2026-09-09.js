...
const catalog={scene4:{label:'シーン4',detail:'二人が中央へ',subject:{kind:'sceneRef',id:'fixture-scene-4'}},lx12:{label:'LX 12',detail:'照明を落とす',subject:{kind:'cueRef',id:'fixture-cue-lx-12'}}};
const state={version:'original',page:0,annotations:[],selected:null,pending:null,mode:'with',left:true,right:true,side:'right',zoom:false,scene:3};
let fixture,history=[],lastPage='',drag=null,suppressClick=false;
const snapshot=()=>structuredClone({version:state.version,page:state.page,annotations:state.annotations,selected:state.selected});
const say=text=>{$('status').textContent=text;};
const entry=key=>state.annotations.find(a=>a.key===key);
const rows=()=>fixture.versions[state.version].pages[state.page].rows;
function render(){if(!fixture)return;layout();renderPage();renderMarks();syncSelection();$('undo').disabled=!history.length;$('count').textContent='割当 '+state.annotations.length+'件';$('version').value=state.version;}
function layout(){
...
function renderPage(){
 ...
 rows().forEach(row=>{const b=document.createElement('button');b.className='row-target';b.dataset.row=row.id;b.setAttribute('aria-label','割当先：'+row.text);b.style.top=(row.rect[1]*100)+'%';b.onclick=()=>{if(state.pending)assign(state.pending,row);else say('先に左のシーンかキューを選ぶと、この台詞へ割り当てられます。');};$('row-overlay').append(b);});
...
function renderMarks(){
 ...
 a.forEach(a=>{const b=document.createElement('button');b.className='pin';b.dataset.source=a.key;b.dataset.annotation=a.id;b.textContent=catalog[a.key].label;b.setAttribute('aria-label',catalog[a.key].label+'の割当先を変更');b.setAttribute('aria-pressed',String(state.selected===a.key));group.append(b);});$('marks').append(group);});
 const unresolved=state.annotations.filter(a=>a.anchor.version!==state.version),tray=$('review-tray');tray.hidden=!unresolved.length;tray.replaceChildren();
 ...
 document.querySelectorAll('[data-assigned]').forEach(el=>{const a=entry(el.dataset.assigned);el.textContent=a?(a.anchor.version===state.version?'割当済み：'+(a.anchor.page+1)+'頁':'要付替え：'+fixture.versions[a.anchor.version].label+'の位置'):'未割当';});
}
...
document.addEventListener('keydown',e=>{if(e.isComposing)return;if(e.key==='Escape'){if(drag||state.pending){clearPending();say('取り消しました。割当は変わりません。');}return;}if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='z'&&!e.shiftKey&&!e.target.closest('input,textarea,select')){e.preventDefault();$('undo').click();}});
...
fetch('fixtures/manifest.json').then(r=>{if(!r.ok)throw new Error('fixture');return r.json();}).then(data=>{fixture=data;$('people').setAttribute('transform','translate(0 55)');render();say('左のシーンかキューを台詞へ置いてみてください。');}).catch(()=>{say('仮台本を読み込めませんでした。ローカルサーバーから開き直してください。');document.querySelectorAll('button,select').forEach(b=>b.disabled=true);});
