...
const workspaceChanged=reason=>document.dispatchEvent(new CustomEvent('trial-workspace-change',{detail:{reason}}));
const entry=key=>state.annotations.find(a=>a.key===key);
const rows=()=>fixture.versions[state.version].pages[state.page].rows.concat(state.annotations.filter(a=>a.anchor.version===state.version&&a.anchor.page===state.page&&a.anchor.kind==='point').map(a=>({id:a.anchor.rowId,text:a.anchor.quote,rect:a.anchor.rect,kind:'point'})));
const isLocalPDF=()=>!!fixture?.versions[state.version]?.local;
function assignmentStatus(key){
 const a=entry(key);if(!a)return 'missing';
 const version=fixture.versions[a.anchor.version],page=version?.pages?.[a.anchor.page];
 return a.anchor.version===state.version&&(version?.local&&!!page||page?.rows.some(r=>r.id===a.anchor.rowId))?'assigned':'review';
}
...
function render(){if(!fixture)return;syncCatalog();layout();renderPage();renderMarks();renderHealth();syncSelection();$('undo').disabled=!history.length;$('open-pdf').disabled=false;$('version').value=state.version;$('open-q-sheet').disabled=false;document.dispatchEvent(new Event('fixture-render'));
...
function renderPage(){
 ...
 const total=fixture.versions[state.version].pages.length;$('page-label').textContent=(state.page+1)+' / '+total+'頁';$('previous').disabled=state.page===0;$('next').disabled=state.page===total-1;
 if(k===lastPage)return;lastPage=k;
 $('pdf-text-controls').hidden=true;$('pdf-highlight').hidden=true;
 if(isLocalPDF()){LocalPDF.display(state.version,state.page);$('script-scroll').scrollTop=0;return;}
 globalThis.LocalPDF?.reset();
...
function renderMarks(){
 ...
 if(isLocalPDF())LocalPDF.marks();
 else rows().forEach(row=>{const a=state.annotations.filter(a=>a.anchor.version===state.version&&a.anchor.page===state.page&&a.anchor.rowId===row.id);if(!a.length)return;
 ...
 if(unresolved.length){const p=document.createElement('p');p.textContent='要付替え '+unresolved.length+'件。項目を選び、新しい台詞へ置きます。';tray.append(p);const div=document.createElement('div');div.className='tools';unresolved.forEach(a=>{const b=document.createElement('button');b.dataset.source=a.key;b.className='pin';b.textContent=catalog[a.key].label;b.title='元の位置：'+(fixture.versions[a.anchor.version]?.label||'読み込めない版')+' '+(a.anchor.page+1)+'頁 '+a.anchor.quote;div.append(b);});tray.append(div);}
 document.querySelectorAll('[data-assigned]').forEach(el=>{const a=entry(el.dataset.assigned);el.textContent=a?(assignmentStatus(a.key)==='assigned'?'割当済み：'+(a.anchor.page+1)+'頁':'要付替え：'+(fixture.versions[a.anchor.version]?.label||'読み込めない版')+'の位置'):'未割当';});
}
...
function assign(key,row){
 ...
 history.push(snapshot());const annotation={id:old?.id||crypto.randomUUID(),key,subject:structuredClone(catalog[key].subject),anchor:{version:state.version,page:state.page,rowId:row.id,quote:row.text,rect:[...row.rect],...(row.kind?{kind:row.kind}:{})},meaningReview:'unconfirmed'};
 if(old)state.annotations=state.annotations.map(a=>a.id===old.id?annotation:a);else state.annotations.push(annotation);
 state.selected=key;state.pending=null;render();workspaceChanged('assignment');say(catalog[key].label+'を「'+row.text+'」へ'+(old?'付け直しました。':'割り当てました。')+' 元に戻すことができます。');
}
function cancelDrag(message){
 ...
 $('ghost').hidden=true;$('workspace').classList.remove('dragging');document.querySelectorAll('.candidate').forEach(el=>el.classList.remove('candidate'));syncSelection();if(isLocalPDF())LocalPDF.marks();if(d.active&&message)say(message);
}
function clearPending(){cancelDrag('ドラッグを取り消しました。割当は変わりません。');state.pending=null;syncSelection();}
function setMode(mode){clearPending();state.mode=mode;if(mode==='with'){state.right=true;state.side='right';}render();workspaceChanged('mode');say(mode==='with'?'台本へ戻りました。割当と頁を保持しています。':'台本を隠しました。割当は保持しています。');}
function showScene(){clearPending();state.scene=4;$('scene-title').textContent='シーン4：二人が中央へ';$('people').setAttribute('transform','translate(0 0)');$('stage-title').textContent='二人が舞台中央に立つ仮の配置';$('scene-strip').innerHTML='表示中<strong>シーン4</strong>二人が中央へ';if($('workspace').clientWidth<760){state.left=false;state.right=false;}render();workspaceChanged('scene');say('シーン4を舞台で表示しました。合図の実行はありません。');}
function captureWorkspace(){
 return structuredClone({schema:1,cues:state.cues,attentionOnly:state.attentionOnly,version:state.version,page:state.page,annotations:state.annotations,mode:state.mode,left:state.left,right:state.right,side:state.side,zoom:state.zoom,scene:state.scene});
}
function restoreWorkspace(raw){
 if(!fixture||!raw||raw.schema!==1||!Array.isArray(raw.cues)||!Array.isArray(raw.annotations)||raw.cues.length>200)return false;
 const unique=new Set();
 if(!raw.cues.every(c=>c&&typeof c==='object'&&typeof c.id==='string'&&typeof c.key==='string'&&typeof c.number==='string'&&typeof c.type==='string'&&typeof c.description==='string'&&!unique.has(c.key)&&unique.add(c.key)))return false;
 const validKeys=new Set(['scene4',...raw.cues.map(c=>c.key)]);
 if(!raw.annotations.every(a=>a&&typeof a==='object'&&typeof a.id==='string'&&validKeys.has(a.key)&&a.subject&&typeof a.subject==='object'&&a.anchor&&typeof a.anchor==='object'&&typeof a.anchor.version==='string'&&Number.isInteger(a.anchor.page)&&a.anchor.page>=0&&typeof a.anchor.rowId==='string'&&typeof a.anchor.quote==='string'&&Array.isArray(a.anchor.rect)&&a.anchor.rect.length===4&&a.anchor.rect.every(Number.isFinite)))return false;
 state.cues=structuredClone(raw.cues);state.annotations=structuredClone(raw.annotations);state.attentionOnly=Boolean(raw.attentionOnly);state.mode=raw.mode==='without'?'without':'with';state.left=Boolean(raw.left);state.right=Boolean(raw.right);state.side=raw.side==='left'?'left':'right';state.zoom=Boolean(raw.zoom);state.scene=raw.scene===4?4:3;
 state.version=typeof raw.version==='string'&&fixture.versions[raw.version]?raw.version:'original';state.page=Math.min(Math.max(0,Number.isInteger(raw.page)?raw.page:0),fixture.versions[state.version].pages.length-1);state.selected=null;state.pending=null;history=[];lastPage='';render();return true;
}
globalThis.TrialState={capture:captureWorkspace,restore:restoreWorkspace,changed:workspaceChanged,ready:()=>Boolean(fixture)};
$('attention-filter').onclick=()=>{clearPending();state.attentionOnly=!state.attentionOnly;render();workspaceChanged('attention');};
$('check-missing').onclick=()=>{clearPending();state.attentionOnly=true;state.left=true;state.side='left';render();workspaceChanged('attention');const first=document.querySelector('#menu [data-source]:not([hidden])');first?.focus({preventScroll:true});say('未割当・要付替えの項目を表示しています。項目を台詞へ置いてください。');};
...
$('version').onchange=e=>{clearPending();const version=e.target.value;if(version===state.version)return;history.push(snapshot());state.version=version;state.page=Math.min(state.page,fixture.versions[version].pages.length-1);render();workspaceChanged('version');say(fixture.versions[version].label+'を表示しました。以前の割当は要付替えとして残ります。');};
$('zoom').onclick=()=>{cancelDrag('表示変更のためドラッグを取り消しました。');state.zoom=!state.zoom;render();workspaceChanged('zoom');};
function page(n){cancelDrag('頁変更のためドラッグを取り消しました。');state.page=n;render();workspaceChanged('page');}
$('previous').onclick=()=>page(Math.max(0,state.page-1));$('next').onclick=()=>page(Math.min(fixture.versions[state.version].pages.length-1,state.page+1));
$('undo').onclick=()=>{clearPending();const old=history.pop();if(!old)return;Object.assign(state,old);render();workspaceChanged('undo');say('直前の割当・付替え・版の変更を戻しました。');};
...
document.addEventListener('pointermove',e=>{
 ...
 const target=document.elementFromPoint(e.clientX,e.clientY)?.closest('[data-row],[data-pdf-surface]');d.candidate=target?.dataset.pdfSurface?LocalPDF.hit(e.clientX,e.clientY):rows().find(r=>r.id===target?.dataset.row)||null;
 if(isLocalPDF())LocalPDF.highlight(d.candidate?.rect||null);
 document.querySelectorAll('[data-row]').forEach(b=>b.classList.toggle('candidate',b===target));
 const ghost=$('ghost');ghost.hidden=false;ghost.textContent=catalog[d.key].label+(d.candidate?' → '+d.candidate.text:' → 台詞へ置く');
...
document.addEventListener('pointerup',e=>{
 if(!drag||drag.id!==e.pointerId)return;const d=drag;const target=document.elementFromPoint(e.clientX,e.clientY)?.closest('[data-row],[data-pdf-surface]');const row=target?.dataset.pdfSurface?LocalPDF.hit(e.clientX,e.clientY):rows().find(r=>r.id===target?.dataset.row);
...
document.addEventListener('keydown',e=>{if(e.isComposing||document.querySelector('dialog[open]'))return;if(e.key==='Escape'){if(drag||state.pending){clearPending();say('取り消しました。割当は変わりません。');}return;}if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='z'&&!e.shiftKey&&!e.target.closest('input,textarea,select')){e.preventDefault();$('undo').click();}});
...
fetch('fixtures/manifest.json').then(r=>{if(!r.ok)throw new Error('fixture');return r.json();}).then(data=>{fixture=data;$('people').setAttribute('transform','translate(0 55)');render();document.dispatchEvent(new Event('trial-ready'));say('割当道具のシーンかキューを台詞へ置いてみてください。');}).catch(()=>{say('仮台本を読み込めませんでした。ローカルサーバーから開き直してください。');document.querySelectorAll('button,select').forEach(b=>b.disabled=true);});
