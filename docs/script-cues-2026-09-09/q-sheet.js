...
(() => {
 ...
 function apply(){
  ...
  history.push(snapshot());state.cues=structuredClone(draft);close();render();TrialState.changed('q-sheet');say('Qシートの変更を反映しました。台本との接続を保持し、元に戻すこともできます。');
 }
 function jumpToScript(c){
  ...
  if(isLocalPDF())LocalPDF.reveal(c.key);else setTimeout(()=>{const row=Array.from(document.querySelectorAll('[data-row]')).find(b=>b.dataset.row===a.anchor.rowId);row?.focus({preventScroll:true});row?.scrollIntoView({block:'center',inline:'nearest',behavior:'instant'});},180);
 ...
 $('q-file').onchange=async()=>{
  ...
  try{const bytes=await file.arrayBuffer();let text,encoding='UTF-8';try{text=new TextDecoder('utf-8',{fatal:true}).decode(bytes);}catch{encoding='Shift_JIS';text=new TextDecoder('shift_jis',{fatal:true}).decode(bytes);}if(token!==epoch||!dialog.open)return;
   const header=QSheetCSV.parse(text)[0]||[];
   if(header.includes('Q番号')&&header.includes('内容')&&header.every(h=>QSheetCSV.headers.includes(h))){const result=QSheetCSV.read(text,draft);draft=result.cues;renderBody();message('CSVの候補：追加 '+result.created+'件 / 更新 '+result.updated+'件。表で確認して反映してください。CSVにないQと台本位置は保持します。');}
   else CSVMigration.open(text,draft,file.name,result=>{if(token!==epoch||!dialog.open)return;draft=result.cues;renderBody();message('移行候補 '+result.additions.length+'件を表に追加しました。確認して「変更を反映」してください。');},encoding);}
