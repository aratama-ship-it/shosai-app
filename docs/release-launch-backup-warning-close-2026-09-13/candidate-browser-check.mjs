...
for (const browserType of [chromium, webkit]) {
  ...
  const closeBox = await regular.page.locator("#stage-launch-backup-close").boundingBox();
  assert.ok(closeBox && closeBox.width >= 44 && closeBox.height >= 44);
  await regular.page.locator("#stage-launch-backup-close").click();
  assert.equal(await modal.isVisible(), false, `${browserType.name()} closes with the top-right button`);
  ...
  assert.equal(await modal.isVisible(), false, `${browserType.name()} closes after an outside click`);
  await regular.page.reload({ waitUntil: "networkidle" });
  assert.equal(await modal.isVisible(), true, `${browserType.name()} returns after outside-dismiss and reload`);
