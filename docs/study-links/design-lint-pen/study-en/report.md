...
| ID | 項目 | 判定 | 実測 | 基準 | 出典 |
|---|---|---|---|---|---|
| C1 | 文字と背景のコントラスト比 | OK | 最悪 7.67:1／実数測定 24件／WARN 0件／無効UI部品 4件は対象外 | 本文 4.5:1以上／大きい文字・UI部品 3:1以上 | §12-16 |
...
| T2 | 行長 | 情報 | 2.5〜24.3字/行（近似）／範囲外 6件 | 1行 30〜50字（全角換算・近似値） | §12-15 |
...
| C3 | 色だけに頼ったリンク表現 | OK | 手がかりなし 0件／本文中リンク 0件 | 本文中リンクに色以外の手がかり | §12-17 |
| T1 | 行送り | OK | 1.60〜1.60／normal等の数値化不可 0件 | line-height / font-size = 1.5〜2.0 | §12-15 |
| T2 | 行長 | 情報 | 2.5〜73.0字/行（近似）／範囲外 6件 | 1行 30〜50字（全角換算・近似値） | §12-15 |
| T3 | ジャンプ率 | 情報 | 1.71倍（24.0px / 本文 14.0px） | 最大見出し / 本文 ≒ 2倍 | §12-15 |
| T4 | 書体の種類数 | OK | 1種類: system-ui | 1〜2種類 | §12-18 |
| U1 | タップ対象の大きさ | OK | 44px未満 0件／対象 13件 | 44×44px以上 | §12-21 |
| U2 | ナビゲーション項目数 | 情報 | ナビゲーションなし | 4〜5以内 | §12-24 |
