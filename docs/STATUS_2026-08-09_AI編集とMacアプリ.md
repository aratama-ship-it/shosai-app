...
## 完了の追記（2026-08-10 本人確認済み）

- 盤面への下書き重ね描き: 実装・commit済み
- 「採る」後に保存へ進まない不具合: 原因（CLI未終了でrunAgent待ち）を特定し修正。
  `--diagnose-adopt` で無人再現可
- データ移行: 公開版へpush後、本人が「書き出す→読み込む」を実行し完了
- commit/push: 6a28efa まで公開済み（GitHub Pages反映確認済み）
- **本人が実機で一往復（指示→下書き→採る→編集）を確認済み**

## 未着手（次の候補）

1. **聞き返しへの返答経路** — needs_clarification の質問に答えると続く形。
   仕様は未確定（会話履歴を持たない方針との整合を決めてから実装）
