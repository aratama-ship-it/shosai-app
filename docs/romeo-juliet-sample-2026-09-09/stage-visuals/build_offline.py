...
class Embed(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        attrs = dict(attrs)
        if tag == 'img' and attrs.get('src') and not attrs['src'].startswith('data:image/'):
...
function handleSavedLink(event){
 ...
 else if(a.ownerDocument!==document && (a.getAttribute('href')||'').startsWith('#')){
  event.preventDefault();
  const target=a.ownerDocument.getElementById(decodeURIComponent(a.getAttribute('href').slice(1)));
