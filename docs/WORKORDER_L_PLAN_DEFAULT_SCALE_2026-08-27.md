...
**★2026-08-27 追記・訂正（Claude）**: 下の表の `front` と `round` は、**Claudeの手計算が
間違っていた**（`M = 24` とすべきところを 12 で計算していた）。Codexが実装時に
食い違いを検出し、指示どおり**表ではなく式のほうを採用**した。**式が正しい。**

| 会場 | いまの `sw` | 当初表（誤） | **実測（正）** | 倍率 |
|---|---|---|---|---|
| 12×9m `front` | 682.7 | ~~784~~ | **768.0** | **1.125倍** |
| 12×9m `none` | 682.7 | 869.6 | **869.6**（横で頭打ち） | **1.274倍** |
| 正方形 `round` | 368 | ~~450~~ | **426.0** | **1.158倍** |

**「1.2倍」は会場によって届かない**（`front` は下の客席の帯が固定費として効くため）。
本人には**「およそ1.13〜1.27倍。会場形式で変わる」**と報告する。
