...
> **この発注は完了している（2026-08-09実装・検証、2026-08-10に狭い画面を追い込み）。**
> 以降は「なぜこう作ったか」を読むための記録として扱う。**再実行しないこと。**
> 実装後に実機で見つけて直した項目と、守るべき境界は
> [LIGHT_INTENT_CARD_HANDOFF.md](LIGHT_INTENT_CARD_HANDOFF.md) にまとめてある。
> 発注書のとおりに作らなかった箇所が数点あるので、**現物のコードを正とすること。**
> 主な差分: 寄せる下限は1100pxではなく900px（カード幅は900〜1099pxで340px、1100px以上で380px）／
> スマホの見る専用画面ではカード・照合帯・重ねを出さない／
> 装置駒は沈むレイヤーと一緒に沈める／視線の線は演者がrevealのときだけ引く。
...
- ブラウザ側テストが **失敗0件** であること。
  ★件数を条件にしない（この発注書は当初「177件」と書いていたが、実際の開始時点は180件で、
  並行セッションが同日中にテストを足していた。**総数の一致を条件にすると突き合わせで手が止まる**）。
  見るのは「失敗0件か」と「自分の変更で落ちたものが無いか」。
- 新規テストが全部通ること
...
- MCP側も **失敗0件** であること（`mcp-server/` は変更しないので当然通るはず。
  通らなければ何かを壊している）。ここも件数は条件にしない。
