...
function compile(b){
 ...
 return {...b,axis,right,up,tan:Math.tan(clamp(b.deg,1,150)*Math.PI/360),rgb:rgb(b.color),
...
function render(ctx,P,dims,kind,beams,people,paintPerson,hazeValue,quick=false){
 const steps=quick?12:18,width=quick?96:192,height=Math.max(32,Math.round(width*ctx.canvas.height/ctx.canvas.width));
 let cv=canvases.get(ctx.canvas);if(!cv){cv=document.createElement('canvas');canvases.set(ctx.canvas,cv);}cv.width=width;cv.height=height;
 const c=cv.getContext('2d'),image=c.createImageData(width,height),data=image.data;
  ...
  if(sigma>0){
   ...
   if(Math.abs(det)>1e-8){
    ...
    for(let iy=0;iy<height;iy++)for(let ix=0;ix<width;ix++){
     ...
     for(const item of bounded){const bb=item.bounds;if(ix<bb[0]||iy<bb[1]||ix>=bb[2]||iy>=bb[3])continue;
