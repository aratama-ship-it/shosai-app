...
const key='shosai.lightDesigns.v1',legacy='shosai.lightDesigns.optionB.v1';
(async()=>{const report={};for(const [engine,type] of Object.entries({chromium,webkit})){
 const browser=await type.launch({headless:true});try{
  ...
  await page.goto('http://127.0.0.1:8813/candidate/?example=back');await page.waitForFunction(()=>window.__RIG?.hooks);
 ...
 }finally{await browser.close();await fs.writeFile(path.join(__dirname,'evidence/storage.json'),JSON.stringify(report,null,2));}
