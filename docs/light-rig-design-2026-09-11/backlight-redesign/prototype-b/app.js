...
(function () {
  ...
  const DESIGN_STORE = "shosai.lightDesigns.optionB.v1";
  ...
  function buildDesign(name) {
    return {
      ...
      app: "照明デザインモード（案B 比較試作）", variant: "option-b",
      stage: { ...state.dims },
      coords: {
  ...
  function openDesigns() {
    ...
    dialog(`<p class="kicker">照明デザインを保存する</p>
      ...
      <p class="hint">${stored===null?'保存一覧を読めないため、ブラウザへの上書きを止めています。':''}案B専用の保存領域です。別の環境へ渡すときはファイルにします。
      もやを含むLX cueの設定を保存します。案Bの読み込みで再現できます。製品本体での再現は未対応です。
