...
## 改訂（2026-08-28 本人フィードバック反映）

検収後の本人指示「望遠は不要。広角より少しだけ広い超広角が欲しい」により構成を変更。

- 新構成: **超広角 120°（新設）／広角 110°／標準 86°（既定・従来どおり）**。望遠(50°)は廃止。
- 保存済みの旧値 `"tele"` は `normalizeLensId` が標準へ倒す（ブラウザ実測で退避を確認済み）。
- 小さな差分のためCodexを介さずClaudeが直接実装。テスト更新・版上げ
  （first-person v18 / i18n v87 / CACHE v164）・stage.html再生成まで実施し、
  実ブラウザで超広角 focal 370（理論値369.5 @1280px幅）を確認した。
