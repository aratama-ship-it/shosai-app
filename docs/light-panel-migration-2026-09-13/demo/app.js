...
(function () {
  ...
  const uid = (p) => {
    const used = new Set([...state.rig.trusses, ...state.rig.fixtures, ...state.scenes.flatMap(s=>s.cue.groups)].map(x=>x.id));
    let value; do { value=`${p}${state.seq++}`; } while(used.has(value)); return value;
  };

  /* ---------- 履歴（モーダル内Undo） ---------- */
  const snapshot = () => JSON.stringify({ rig: state.rig, scenes: state.scenes, palette: state.palette, levelCurve: state.levelCurve, curtains: state.curtains, dims: state.dims, migration: state.migration || null, designName: state.designName || "", sceneIndex: state.sceneIndex });
  ...
  function restore(json) {
    ...
    if (o.dims) state.dims = o.dims;
    state.migration = o.migration || null; state.designName = o.designName || "";
    state.sceneIndex = Math.max(0, Math.min(o.sceneIndex || 0, state.scenes.length - 1));
    state.nextNo = Math.max(0, ...state.rig.fixtures.map(f => f.no)) + 1;
    ...
    if (o.curtains) state.curtains = o.curtains;
  ...
  const houseProjPlan = (P, B) => (w) => { const q = P(w); if (w && w.y > state.dims.D + 1e-6) q.Y = B.y + B.h + HOUSE_DY; return q; };
  const houseProjSide = (P) => (w) => {
    const q = P(w); const D = state.dims.D;
    if (w && w.y > D + 1e-6) { const fx = P({ x: 0, y: D, z: 0 }).X, bx = P({ x: 0, y: 0, z: 0 }).X; q.X = fx + Math.sign(fx - bx || 1) * FRONT_DX_SEC; }
    return q;
  };
  ...
  function ensureOn(fid) {
    ...
    if (f?.mount?.type === "legacy-panel" && ![f.mount.u,f.mount.v,f.mount.h].every(Number.isFinite)) { toast("配置タブで光源の位置を設定してください。元のショーには位置の記録がありません"); return; }
    if (l?.path) { if (l.on !== true) setLight(fid, { on: true }); return; }
    const a = defaultAim(f); setLight(fid, { on: true, surface: a.surface, path: { kind: "still", a: a.a }, speed: "normal", color: (l && l.color) || COLORS[0] });
  }
  ...
  const groupName = (g) => (g.legacyGroupId && g.name ? g.name + "・" : "") + ({ together: "一緒に動く", mirror: "鏡のように動く", sequential: "順番に動く" }[g.relation] || "") + (g.compose === "fan" ? "（扇）" : g.compose === "cross" ? "（交差）" : "");
      ...
      state.rig.fixtures.forEach((f) => {
        ...
        if (l.surface === "floor" || l.surface === "air") {
          ...
          /* 客席へ向けた光。狙い点は客席帯に置き（実距離は数値）、光だまりは描かない——
             観客からは光源そのものが見える「目眩まし」なので、狙い点のまわりにふわっと滲ませるだけにする
             （2026-09-13 本人要望「場所を間違えると目眩まし的なエフェクトになる。それもふわっと」）。 */
          const th = PH(T);
          if (showOn("beam")) {
  ...
  function applyHandleDrag(dg, values, axis) {
    ...
    targets.forEach((h) => {
      ...
      if (l.surface === "house") target.v = 1;   // 客席へ向けた点は手前端に固定（客席側の距離は数値で決める）
    });
  }
  ...
  function mountSections(from) {
    ...
    secs.push({ key: "legacy-panel", name: "旧パネルから引き継いだ照明", items: list.filter(f => f.mount.type === "legacy-panel") });
  ...
  function renderList() {
    ...
    const row = (f, idx) => { const r = document.createElement("div"); r.className = "row" + (isSel(f.id) ? " sel" : ""); const st = lightState(f.id);
      r.innerHTML = `<span class="no">${idx !== undefined ? idx + 1 + "." : ""}${label(f.id)}</span><span class="nm">${escapeHTML(f.name || "名前なし")}<small>${E.describeMount(f, state.rig).replace(/（高さ約\dm）/, "")}</small></span>`;
  ...
  function renderFixedConflicts() {
    ...
    list.forEach(({ f, variants }) => {
      ...
      variants.forEach((v) => {
        ...
        b.title = `当てる先＝${l.surface === "air" ? "空中" : l.surface === "back" ? "奥の壁" : l.surface === "house" ? "客席" : "床"}／広がり${Math.round(E.beamDegOf(f, l))}°`;
        acts.append(b);
      });
  ...
  function renderInspector() {
    ...
    if (state.mode === "place") {
      ...
      if (ids.length === 1) {
        ...
        if (m.type === "legacy-panel") {
          host.append(el("p", "note", "旧パネルに保存された光源の座標です。未設定の欄は記録がありません。3欄を入力すると点灯できます。"));
          if (f.legacyPanel?.note) host.append(el("p", "note", "旧パネルのメモ：" + f.legacyPanel.note));
          for (const [key, title, min, max] of [["u","光源の横位置（割合）",-.3,1.3],["v","光源の奥行き（割合）",-.3,1.3],["h","光源の高さ（m）",0,18]]) {
            const input = document.createElement("input"); input.type="number"; input.min=min; input.max=max; input.step="any"; input.value=Number.isFinite(m[key]) ? m[key] : ""; input.placeholder="未設定";
            input.onchange=()=>{ const n=Number(input.value); if(input.value!=="" && Number.isFinite(n) && n>=min && n<=max){m[key]=n; commit();} else {input.value=Number.isFinite(m[key])?m[key]:"";toast("範囲内の数値を入力してください");} };
            host.append(field(title,input));
          }
        }
    ...
    if (ids.length === 1) {
      ...
      const g = groupOf(fid);
      if (g) { const gi = cue().groups.indexOf(g); const box = el("div", "box", `<p class="hint">組${gi + 1}「${escapeHTML(groupName(g))}」の一員です。</p>`); box.append(btn("組から外す", () => ungroup(fid), "small quiet")); host.append(box); }
      ...
      if (f.mount.type === "cyc") {
        ...
        if (f.mount.type === "legacy-panel") {
          b.append(field("この場面の広がり", range(4,70,1,E.beamDegOf(f,l),fmtDeg,v=>{l.beamDeg=v;draw();},()=>commit()),true));
          b.append(el("p","note","旧パネルの直径から、この場面の距離に合わせて換算した値です。元の直径は移行記録に残ります。"));
        } else if (mover) {
  ...
  $("apply").hidden = true; // This isolated preview cannot apply changes to a product show.
  $("apply").onclick = openDesigns;
  $("close").onclick = openDesigns;
  ...
  const DESIGN_STORE = "shosai.lightDesignMigration.preview.v1";

  function buildDesign(name) {
    return {
      format: DESIGN_FORMAT, version: state.migration ? DESIGN_VERSION : 1,
      ...(state.migration ? { migration: JSON.parse(JSON.stringify(state.migration)) } : {}),
  ...
  function applyDesign(input) {
    const o = window.STAGE_LIGHT_PANEL_MIGRATION.validateDesign(input);
    const previous = snapshot(), wasDirty = state.dirty;
    try {
      state.dims = { ...o.stage };
      state.scenes = o.scenes.map(ds => ({ id: ds.id, name: ds.name, pieces: [], cue: ds.cue }));
      state.rig = o.rig; state.migration = o.migration || null;
      state.palette = Array.isArray(o.palette) ? [...o.palette] : [];
      state.levelCurve = o.levelCurve?.length === LEVEL_CURVE_POINTS ? [...o.levelCurve] : [0,.25,.5,.75,1];
      state.curtains = { ...state.curtains, ...(o.curtains || {}), pieces: o.curtains?.pieces || [] };
      state.nextNo = Math.max(0,...state.rig.fixtures.map(f=>f.no)) + 1;
      state.sceneIndex=0; state.sel.clear(); state.selTruss=null; state.collapsed.clear(); state.search=""; state.filter="all";
      state.designName = o.name || "";
      renderAll();
    } catch (e) { restore(previous); state.dirty=wasDirty; throw e; }
    state.history.push(previous); state.future=[]; state.dirty=true; baseline=snapshot();
    renderAll();
    window.dispatchEvent(new Event("lighting-design-change"));
  }
  const readStore = () => { try { const raw = localStorage.getItem(DESIGN_STORE); const a = raw ? JSON.parse(raw) : []; if (!Array.isArray(a)) throw new Error("形式不明"); return a; } catch (e) { toast("保存されたデザインを読めません。上書きせず停止しました。移行内容・元データから元ショーを書き出せます"); return null; } };
  ...
  function openDesigns() {
    const list = readStore(); if (!list) return;
    const rows = list.length
      ? list.map((d, i) => `<div class="dsrow" data-i="${i}">
          <span class="dsname">${escapeHTML(d.name || "名前なし")}<small>${escapeHTML(String(d.savedAt || "").slice(0, 16).replace("T", " "))}　灯${(d.rig && d.rig.fixtures ? d.rig.fixtures.length : 0)}・場面${(d.scenes || []).length}</small></span>
          <span class="dsacts">
    ...
    dialog(`<p class="kicker">照明デザインを保存する</p>
      ...
      <p class="hint">保存先はこのローカル確認版専用です。移行したデザインのファイルには、復元用の元ショー全体も含みます。人へ渡すときはその点をご確認ください。舞台スケッチ本体への取り込みは、統合時に対応します。</p>
    ...
    if ($("dspick")) $("dspick").onchange = async () => {
      ...
      try { const source = await f.text(); const value = JSON.parse(source);
        if (value.format !== DESIGN_FORMAT) { $("dialog").hidden=true; window.LIGHT_MIGRATION_UI.review(value, source); return; }
        applyDesign(value); $("dialog").hidden = true; toast(`「${state.designName || f.name}」を読み込みました`); }
  ...
  window.__RIG = { state, E, planBox, secBox, secOf, SECS, applyDesign, buildDesign, snapshot, undo, redo, ensureOn, setLight, commit, renderAll, download };
