...
(function (root) {
  ...
  const beamDegOf = (fixture, light) => {
    ...
    if ((!isMoving(fixture) && fixture?.mount?.type !== "legacy-panel") || !light || light.beamDeg == null) return base;
  ...
  const fixtureWorld = (fixture, rig, dims = DEFAULT_DIMS) => {
    ...
    if (m.type === "legacy-panel") {
      ...
      return { x: (m.u - .5) * dims.W, y: m.v * dims.D, z: m.h };
    }
    if (m.type === "truss") {
  ...
  const pointWorld = (p, dims) => p?.coordinateMode === "legacy-panel" && [p.u,p.v,p.hM].every(Number.isFinite)
    ? { x: (p.u-.5)*dims.W, y:p.v*dims.D, z:p.hM } : ({
  ...
  const describeMount = (fixture, rig) => {
    const m = fixture.mount || {};
    if (m.type === "legacy-panel") return Number.isFinite(m.h) ? `旧パネルの配置・高さ${m.h}m` : "旧パネル・位置未設定";
