...
(function (root) {
  ...
  function sourceProject(doc) {
    ...
    for (const s of p.scenes) {
      ...
      unique(array(s.pieces, "場面の配置"), "配置");
  ...
  function validateDesign(value) {
    ...
    for (const f of d.rig.fixtures) {
      ...
      if (f.mount.type === "legacy-panel") {
        ...
        const m = f.mount;
        if (m.u !== null) number(m.u, -.3, 1.3, "光源の横位置"); if (m.v !== null) number(m.v, -.3, 1.3, "光源の奥行き"); if (m.h !== null) number(m.h, 0, 18, "光源の高さ");
      }
    }
  ...
  function migrate(input, options = {}) {
    ...
    function setting(piece, reg, context) {
      ...
      const mount = { type: "legacy-panel", u: num(b.u, Math.max(0, Math.min(1, u)), -.3, 1.3, "光源の横位置", context), v: num(b.v, Math.max(0, Math.min(1, v)), -.3, 1.3, "光源の奥行き", context), h: num(b.h, 6, 0, 18, "光源の高さ", context) };
