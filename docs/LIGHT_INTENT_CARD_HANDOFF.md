...
**★件数を完了条件にしない。** 複数セッションが同時にテストを足すので総数は動く。
下はPhase 1時点の記録で、同じ日のうちに180/33へ増えている。
**この数字を「あるべき件数」として渡すと、突き合わせで手が止まる**（2026-08-09に実際に発生）。
見るべきは「失敗0件か」であって総数ではない。現在値は毎回コマンドで確認する。

2026-08-09のローカル確認結果（Phase 1時点）:

- ブラウザ側: 177件中177件成功（記録時点）
- MCP側: 25件中25件成功（記録時点）
...
6. **スマホの見る専用画面で、要約の行がシーン送りの上に重なっていた。**
   カードを `.stage-scene-bar` の外へ出したため、作り手用の帯を前提に組んである
   スマホ用レイアウトで座標が衝突し、**前へ／次へが押せなくなっていた**。
   スマホは「JSONを読み込んで見る」画面で、シーン一覧・道具箱・インスペクタを既に
   全部隠している。カードと照合帯も同じ非表示リストへ入れ、
   `lightIntentOverlayOn()` でも図の上の重ねを止めた（あちらには切るトグルが無いため）。
7. **変更した資産の `?v=` が上がっていなかった。** 実際に古いCSSが配信された。
