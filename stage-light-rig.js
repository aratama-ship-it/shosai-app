/* 「照明を組む」— 幾何と時間の純関数（DOM・描画を持たない）
 *
 * 位置づけ: 舞台スケッチ本体の中の一画面。灯体をバトン・前明かり・SS・転がしへ置き、
 *   一灯ずつ当てる先と動きを決めて、照明担当者へ渡す「案」を作る。
 *   機材の型番・回路・DMX・照度は持たない。
 *
 * 設計と根拠は docs/light-rig-design-2026-09-11/index.html（試作・参照調査・実測つき）。
 *   試作: docs/light-rig-design-2026-09-11/prototype/（この幾何はそこから移植）
 *
 * 座標の契約（本体 stage-sketch.js と同じ）:
 *   u: 左右 0..1（0=下手側、1=上手側）／ v: 奥行き 0..1（0=最奥、1=最前＝客席側）
 *   世界座標: x=(u-0.5)*W [m], y=v*D [m], z=高さ [m]
 *   狙い点は Point3 = {u, v, hM}（hM=床からの高さm）。「当てる場所」(floor/back/air)は
 *   その点への制約であって、データの形は変えない。
 *
 * 語彙は本体の照明パネルに合わせる（index.html の select が正）:
 *   吊り（バトンから真下へ）／SS（袖から横切って）／前明かり（客席の上から顔へ）／転がし（床置きから体へ）
 *   吊る棒は「バトン」。「トラス」とは呼ばない。
 *
 * 正面図の3D（擬似パース）は**本体の描画を使う**（window.SHOSAI_STAGE_VIEW ごしに借りる）。
 *   この中に写さない——本体を直したときにズレるため。試作にある FRONT_SEATS 等は移植していない。
 */
(function (root) {
  "use strict";
  ...
  /* 1往復（1周）にかける時間。3段（ゆっくり／普通／速い）より、
     秒で直に決めた periodSec を優先する（2026-09-12 本人要望）。 */
  const periodMs = (light) => {
    const sec = Number(light && light.periodSec);
    if (Number.isFinite(sec) && sec > 0) return clamp(sec, 0.2, 120) * 1000;
  ...
  const targetAt = (light, cue, fixtureId, tMs, dims = DEFAULT_DIMS) => {
    ...
    /* 灯ごとに何秒遅らせて始めるか。組の「順番に動く」とは別で、同じ動きを
       少しずつずらして波を作るのに使う（サーチライト。2026-09-12 本人要望）。 */
  ...
  root.SHOSAI_STAGE_LIGHT_RIG = Object.freeze({
