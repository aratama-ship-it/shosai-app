...
for (const [code, localName] of [["zh-Hans", "舞台速写"], ["zh-Hant", "舞台速寫"]]) {
  test(`${code}パックはドラフトを保ち、画像・印刷・全画面表記・打楽器2種・仮面を反映する`, () => {
    ...
    assert.equal(Object.keys(pack.text).length, 1050 + Object.keys(viewSelectLabels).length + Object.keys(panelWidthLabels).length + Object.keys(skinLabels).length + Object.keys(aboutFeedbackLabels).length + Object.keys(releaseHistoryLabels).length + Object.keys(releaseNotificationLabels).length + Object.keys(exportLabels).length + Object.keys(projectExportLabels).length + Object.keys(projectSettingsLabels).length + Object.keys(drumLabels).length + Object.keys(drawerLabels).length + Object.keys(maskLabels).length + Object.keys(batchUiLabels).length + Object.keys(shapeGroupLabels).length + Object.keys(sceneCreateLabels).length + Object.keys(sectionUiLabels).length + Object.keys(sceneSubtitleLabels).length + Object.keys(transitionLabels).length + Object.keys(panelLayoutLabels).length + Object.keys(panelLayoutResetLabels).length + Object.keys(multiSelectLabels).length + Object.keys(rosterResizeLabels).length + Object.keys(wideVenueTextLabels).length + Object.keys(timelineLabels).length + Object.keys(betaAudioLabels).length + 23);
