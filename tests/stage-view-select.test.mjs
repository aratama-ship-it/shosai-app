...
test("Tで片面図を切り替え、両面表示中は上下順を替える", () => {
  ...
  assert.match(source, /event\.metaKey \|\| event\.ctrlKey \|\| event\.altKey \|\| event\.shiftKey/);
  assert.match(source, /event\.repeat \|\| event\.isComposing \|\| event\.keyCode === 229 \|\| isTyping\(event\.target\)/);
