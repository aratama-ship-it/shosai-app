...
test("受け入れ会場で4本の線・探り針・赤い観客重なりを即時に更新しvenueへ保存しない", () => {
  ...
  assert.ok(editor.drawFills.includes("rgba(238,55,48,0.72)"), "観客との重なりを赤く塗っていない");
...
test("会場ライブラリはfresh対象で、変更JSの版とPWAキャッシュ版が揃う", () => {
  ...
  for (const [name, version] of [
    ["stage-venues.js", "16"],
    ["stage-venue-lines.js", "1"],
    ["stage-sketch.js", "228"],
    ["stage-venue-editor.js", "4"],
  ]) {
  ...
  for (const page of [indexSource, stageHtml]) assert.ok(page.includes("style.css?v=164"));
  assert.ok(swSource.includes("./style.css?v=164"));
  assert.match(swSource, /const CACHE_NAME = "stage-sketch-pwa-v52";/);
});
