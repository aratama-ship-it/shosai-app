...
test("会場ライブラリはfresh対象で、変更JSの版とPWAキャッシュ版が揃う", () => {
  ...
  for (const page of [indexSource, stageHtml]) assert.ok(page.includes("style.css?v=214"));
  assert.ok(swSource.includes("./style.css?v=214"));
  assert.ok(stageHtml.includes("stage-machinery.js?v=2"));
  assert.ok(swSource.includes("./stage-machinery.js?v=2"));
  assert.ok(stageHtml.includes("stage-first-person.js?v=17"));
  assert.ok(swSource.includes("./stage-first-person.js?v=17"));
