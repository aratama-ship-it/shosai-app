...
test("正本・PWAキャッシュの版を更新する", async () => {
  ...
  assert.match(html, /style\.css\?v=369/);
  assert.match(html, /stage-sketch\.js\?v=483/);
  assert.match(html, /stage-i18n\.js\?v=178/);
  assert.match(sw, /stage-sketch-pwa-v489/);
  assert.match(sw, /style\.css\?v=369/);
  assert.match(sw, /stage-sketch\.js\?v=483/);
  assert.match(sw, /stage-i18n\.js\?v=178/);
});
