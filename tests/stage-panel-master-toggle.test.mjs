...
test("正本・PWAキャッシュの版を更新する", async () => {
  ...
  assert.match(html, /style\.css\?v=372/);
  assert.match(html, /stage-sketch\.js\?v=484/);
  assert.match(html, /stage-i18n\.js\?v=179/);
  assert.match(sw, /stage-sketch-pwa-v492/);
  assert.match(sw, /style\.css\?v=372/);
  assert.match(sw, /stage-sketch\.js\?v=484/);
  assert.match(sw, /stage-i18n\.js\?v=179/);
});
