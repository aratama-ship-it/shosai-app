...
test("正本・PWAキャッシュの版を更新する", async () => {
  ...
  assert.match(html, /style\.css\?v=356/);
  assert.match(html, /stage-sketch\.js\?v=469/);
  assert.match(html, /stage-i18n\.js\?v=175/);
  assert.match(sw, /stage-sketch-pwa-v463/);
  assert.match(sw, /style\.css\?v=356/);
  assert.match(sw, /stage-sketch\.js\?v=469/);
