...
test("正本・PWAキャッシュの版を更新する", async () => {
  ...
  assert.match(html, /style\.css\?v=378/);
  assert.match(html, /stage-sketch\.js\?v=490/);
  assert.match(html, /stage-i18n\.js\?v=181/);
  assert.match(sw, /stage-sketch-pwa-v504/);
  assert.match(sw, /style\.css\?v=378/);
  assert.match(sw, /stage-sketch\.js\?v=490/);
  assert.match(sw, /stage-i18n\.js\?v=181/);
});
