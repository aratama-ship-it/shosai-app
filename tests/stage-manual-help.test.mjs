...
test("正本と単独版に検索・冊子導線と共通データが継承される", () => {
  for (const source of [indexSource, stageSource]) {
    ...
    assert.ok(
      source.indexOf("manual/manual-content.js?v=1") < source.indexOf("stage-sketch.js?v=299"),
      "共通マニュアルデータがstage-sketch.jsより先に読み込まれる",
    );
  }
});

test("Service Workerは冊子一式をv154へキャッシュする", () => {
  assert.match(swSource, /const CACHE_NAME = "stage-sketch-pwa-v154";/);
  for (const entry of [
