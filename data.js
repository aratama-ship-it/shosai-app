...
const SHOSAI = {
  ...
  seeds: [
    { recipe: "機能保存・手段交換", dist: "near",
      ...
      sources: ["show_eloize_cirkopolis", "show_cds_varekai"],
      inspiration: "同じ場所の内部規則が変わっていく構造と、落下から回復へ向かう運動を、糸の角度だけで世界の重力が変わる場面へ移した。" },
    { recipe: "時間再編集", dist: "near",
      text: "同じ10秒の技を5回繰り返すが、毎回どこかが1つ欠けていき、観客は5回目に「最初から欠けていた」と気づく。",
      sources: ["show_tanztheater_cafe_muller", "show_immersive_sleep_no_more"],
      inspiration: "反復が関係の見え方を変える構造と、同じ出来事を別の位置から見直す体験を、欠落が増える短い技へ圧縮した。" },
    { recipe: "素材起点", dist: "near",
      text: "濡れた糸は重く、技が遅い。乾くにつれ速くなり、乾き切った瞬間に音が消える。",
      sources: ["show_jp_aqua1_seaparadise_animal_life_live", "show_eloize_rain"],
      inspiration: "水を背景ではなく身体と進行を変える条件として扱い、乾燥の時間を技の速度と音の変化へ置き換えた。" },
    { recipe: "関係再配線", dist: "near",
      text: "演者がディアボロを操るのではなく、客席の咳や衣擦れの音にディアボロが同期して動いているように見える。",
      sources: ["show_immersive_sleep_no_more", "show_xy_les_voyages"],
      inspiration: "観客の位置や振る舞いが体験を変える仕組みを、客席が無意識のまま道具の操作者になる関係へ反転した。" },
    { recipe: "結果先出し", dist: "near",
      text: "開演時、床に散らばった百個の白い破片。60分かけて、それが「何の結果だったのか」へ観客が遡っていく。",
      sources: ["show_gandini_smashed", "show_musical_wicked"],
      inspiration: "多数の物を秩序立てる構成と、結末を先に見せて過去へ戻る枠を合わせ、破片から原因を復元する開幕にした。" },
    ...
    { recipe: "時間再編集（逆順）", dist: "mid",
      ...
      sources: ["show_musical_wicked", "show_musical_phantom_of_the_opera"],
      inspiration: "結末から過去をたどる枠と、現在の物から別の時代を立ち上げる転換を、上演そのものの逆行へ置き換えた。" },
    { recipe: "機能保存・手段交換", dist: "mid",
      text: "大観衆のウェーブが持つ「参加の伝播」を、客席30人の指先だけで起こす。",
      sources: ["show_xy_les_voyages", "show_musical_lion_king"],
      inspiration: "客席まで出来事を広げる参加の力を残し、規模の大きさを指先の連鎖へ交換した。" },
    ...
    { recipe: "関係再配線", dist: "alien",
      ...
      sources: ["show_musical_lion_king", "show_tanztheater_cafe_muller"],
      inspiration: "仕掛けを扱う人や機能的な介入者を隠さず見せる発想を、照明と演者の主従が揺れる関係へ移した。" },
    { recipe: "結果先出し", dist: "alien",
      text: "千秋楽にしか完成しない絵。毎公演、演者の動線がペンキの足跡として床に1層ずつ重なっていく。",
      sources: ["show_baro_les_escapades", "show_casino2_wynn_tree_prosperity_2006"],
      inspiration: "公共空間に徴が現れていく進行と、時間の推移を物の変化で見せる構造を、公演をまたいで育つ床面へ延長した。" },
    ...
    { recipe: "制約強化", dist: "alien",
      ...
      sources: ["show_noh_dojoji", "show_immersive_sleep_no_more"],
      inspiration: "強い約束事を持つ場と、観客が規則を読みながら進む体験を、畳の縁を越えない単純な空間ルールへ縮小した。" },
    { recipe: "視点移植", dist: "near",
      text: "床の影だけが、演者より一拍早くディアボロの技を成功させる。本人は影を追ううちに遅れていく。最後にどちらを本体として残すかは決めていない。",
      sources: ["show_musical_lion_king", "show_musical_phantom_of_the_opera"],
      inspiration: "演者と造形を同時に見せる二重性と、現在から別の時間が立ち上がる仕掛けを、影が未来を先演する関係へ変形した。" },
    { recipe: "失敗の継承", dist: "near",
      text: "落とした道具を拾わない。床に残る失敗が次の技の障害と道標になり、演者の動線だけが少しずつ狭くなる。何個目で戻れなくなるかは試して決める。",
      sources: ["show_gandini_smashed", "show_cirkor_tipping_point"],
      inspiration: "多数の物が作る秩序と、均衡が崩れる臨界を、落下の痕跡が次の運動条件になる連鎖へ置き換えた。" },
    { recipe: "機能の継承（音）", dist: "near",
      text: "キャッチが成功するたび、音楽から楽器が一つ消える。最後は呼吸と糸の摩擦音だけで同じフレーズを続けるが、無音まで行くかは決めていない。",
      sources: ["show_eloize_nomade", "show_eloize_cirque_orchestra"],
      inspiration: "音楽・身体・共同体の密度で時間を運ぶ構造と、生演奏とサーカスの関係を、成功のたび伴奏が身体へ移る減算にした。" },
    { recipe: "因果の逆転", dist: "near",
      text: "投げる直前に、未来のキャッチ音が鳴る。反復するたび音の先行が長くなり、演者はまだ起きていない成功へ追いつこうとする。",
      sources: ["show_musical_phantom_of_the_opera", "show_musical_wicked"],
      inspiration: "後の出来事を先に提示して過去へ戻る時間操作を、未来の結果音だけが現在へ漏れる小さな因果逆転へした。" },
    { recipe: "空間の主役化", dist: "near",
      text: "円形の光は演者ではなくディアボロだけを追う。演者は明るい場所へ入ろうとするが、物が止まるたび光も止まる。暗闇に残す身体の量は未決定。" },
    { recipe: "素材起点（痕跡）", dist: "near",
      text: "白い粉を含んだ糸が、技のたび床と衣服へ軌道を残す。終盤、その線を演者が地図として歩き直すが、どこへ着く地図かは空けておく。" },
    { recipe: "関係再配線（交代）", dist: "near",
      text: "二つのディアボロが交互に回り、一方が止まっている間だけ演者は自由に動ける。やがて二つが同時に回り、身体の逃げ場がなくなる。" },
    { recipe: "尺度交換", dist: "near",
      text: "机上の指先だけで始めた小さな操作を、同じリズムのまま腕、胴体、舞台全体へ拡大する。最大になった後、どの尺度へ戻るかは決めない。" },
    { recipe: "成功の儀式化", dist: "near",
      text: "技が一つ成功するたび、演者は椅子を出口へ一歩近づける。椅子が扉へ着いたとき、退場するのが演者か道具かはまだ決めていない。" },
    { recipe: "認識遅延（拍手）", dist: "near",
      text: "開演直後に録音された大きな拍手が鳴る。以後、何も起きていない瞬間にだけ同じ拍手が戻り、観客は何を見落としたのか探し始める。" },
    { recipe: "制約強化（飛翔中のみ）", dist: "near",
      text: "演者は道具が空中にある間しか歩けない。キャッチすると身体が停止し、次の投げでだけ位置を変えられる。止まることが休息か拘束かは決めない。" },
    { recipe: "時間再編集（三時制）", dist: "near",
      text: "同じ30秒を遅い過去、等速の現在、速い未来として三度行う。四度目は三つのテンポが同じ身体に重なり、どの時間が勝つかは稽古で探す。",
      sources: ["show_eloize_nomade", "show_mv_vanessa_carlton_a_thousand_miles"],
      inspiration: "一夜の時間を密度の変化で運ぶ構造と、同じ行為を保ったまま周囲だけが移動する映像構造を、身体に重なる三つの時制へ移した。" },
    { recipe: "不在の演出", dist: "near",
      text: "演者が退場した後も、回転音と揺れる糸だけが一分間残る。再登場して止めるか、客席が静まるまで待つかは決めていない。" },
    { recipe: "観客プロトコル（環境音）", dist: "near",
      text: "開演時の客席で最初に聞こえた咳、紙、椅子の音を一つだけ拾い、その間隔を演目の最初のテンポにする。観客には選ばれたことを知らせない。" },
    { recipe: "時間の衣装化", dist: "mid",
      text: "一日の進行を衣装の層で見せる。脱いだ布は床に残り、朝は身体を包んでいた色が、夜には舞台の地形へ変わる。翌朝に着直すかは未決定。",
      sources: ["show_eloize_nomade", "show_casino2_wynn_tree_prosperity_2006"],
      inspiration: "夕暮れから夜明けへ進む時間骨格と、季節変化を一本の物へ託す構造を、衣装が地形へ変わる一日の層として組み直した。" },
    { recipe: "役割の可視化", dist: "mid",
      text: "転換スタッフが黒子にならず、落ちた物を同じ歩数と呼吸で回収する。回収の列が次の群舞へ変わる瞬間だけ、誰が出演者か分からなくなる。",
      sources: ["show_musical_lion_king", "show_tanztheater_cafe_muller"],
      inspiration: "操作する人を造形と同時に見せる方法と、物を動かす介入者が反復へ組み込まれる構造を、転換作業の群舞へ移した。" },
    { recipe: "物の連帯", dist: "mid",
      text: "十二個の小道具が一人の演者を順番に拒み、別の演者の足元へ集まっていく。最後にできる物の集団を、味方にするか壁にするかは決めない。" },
    { recipe: "二度目のパレード", dist: "mid",
      text: "同じパレードが二度通る。一度目は人物が祝い、二度目は彼らが落とした紙片、壊れた飾り、疲れた歩幅だけが同じ音楽で戻ってくる。",
      sources: ["show_themepark_main_street_electrical_parade_1972", "show_jp_theme1_shima_buen_viaje_2024"],
      inspiration: "反復可能な光と音のモチーフで隊列をつなぐ方法と、昼夜で表情を変えるパレードの記録を、祝祭の後始末だけが再巡回する構成へ反転した。" },
    { recipe: "時間差の窓", dist: "mid",
      text: "舞台上の動きをライブカメラが七秒遅れで映す。演者は過去の自分と位置を合わせようとし、最後だけ映像のほうが先に動く。",
      sources: ["show_mv_vanessa_carlton_a_thousand_miles", "show_immersive_sleep_no_more"],
      inspiration: "同じ行為を続けながら背景の時間だけが移る構造と、反復を別視点で追う体験を、現在と七秒前が同居する窓へ変えた。" },
    { recipe: "建築への寄生", dist: "mid",
      text: "向かいの建物の窓が一つ点くたび、屋外の演者が一人増える。全窓が明るくなったとき、観客が見るべき場所は地上か建物か決めていない。",
      sources: ["show_jp_theme2_tokyo_night_light", "show_baro_les_escapades"],
      inspiration: "既存建築を夜の舞台面にする方法と、公共空間の徴が増えて人の集まりを変える進行を、窓と演者の増殖関係へ移した。" },
    { recipe: "観客プロトコル（照明）", dist: "mid",
      text: "観客が立つとその足元だけが明るくなり、演者は光の島を渡って移動する。誰も立たない時間にも作品が続く方法を一つ残しておく。" },
    { recipe: "水面の裏表", dist: "mid",
      text: "水面上では祝祭のダンス、水面下の映像では同じ身体が沈んで見える。二つの速度が一致する一瞬を終幕にするか転換点にするかは未決定。",
      sources: ["show_casino_wynn_macau_performance_lake_2006", "show_casino_okada_fountain_symphony_2017"],
      inspiration: "水・光・音で反復される短いスペクタクルと、生身の演者を加えられる水上空間を、水面の上下で同じ身体が二重に見える場面へ変形した。" },
    { recipe: "公演の蓄積", dist: "mid",
      text: "一公演ごとに舞台奥の壁へ一本だけ線を引く。千秋楽では線が風景になるが、初日の観客にも未完成さを作品として渡す。" },
    { recipe: "名前の交換", dist: "mid",
      text: "三人の演者が衣装ではなく名前を交換する。呼ばれた名前の人物が技をする規則が崩れ、観客は身体ではなく反応で人物を見分け始める。" },
    { recipe: "安全装置の転用", dist: "mid",
      text: "落下を受けるネットが、誰も落ちない時間には空、海、天井として扱われる。実際に一度だけ落ちた後、同じ布が何に見えるかを変える。" },
    { recipe: "多視点の同期", dist: "mid",
      text: "円形会場の北側には物語、南側にはその仕掛けが見える。同じ決定的瞬間を、片側は驚き、片側は準備として受け取る構図を作る。",
      sources: ["show_casino_bellagio_fountains_1998", "show_casino_okada_fountain_symphony_2017"],
      inspiration: "広い場所の複数地点から同じ水上演出を見る条件を、片側には像、反対側には準備が見える非対称な客席へ押し進めた。" },
    { recipe: "失敗による分岐", dist: "mid",
      text: "最初の難技が成功した日は祝宴へ、落下した日は捜索へ進む。どちらも同じ終幕へ着くが、観客には分岐したことを説明しない。",
      sources: ["show_gandini_smashed", "show_cds_quidam"],
      inspiration: "落下を含む物の関係と、日常からずれた世界へ進む構造を、最初の成功・失敗が別の物語規則を選ぶ分岐へ移した。" },
    { recipe: "音楽の物質化", dist: "mid",
      text: "生演奏の一音ごとに天井から短い糸が一本降りる。曲が終わる頃には演者の通路が狭まり、アンコールは糸の森の中で始まる。" },
    { recipe: "嗅覚の先行", dist: "alien",
      text: "舞台に何もないうちに雨、木、焦げた紙の匂いだけが順に届く。数分遅れて対応する物が現れるが、最後の匂いだけは物を持たない。" },
    { recipe: "官僚制の振付", dist: "alien",
      text: "跳ぶ、回る、触れるたびに別の演者から許可印をもらう。書類が増えるほど動きは速くなり、規則を守ることが危険に見え始める。" },
    { recipe: "不在の主人公", dist: "alien",
      text: "舞台上の全員が、そこにいない一人を避け、支え、見送る。空白の位置だけが移動し、観客は最後まで姿を見ない。",
      sources: ["show_tanztheater_cafe_muller", "show_7f_en_panne"],
      inspiration: "反復される身体関係と、空になった場所そのものを作品の条件にする発想を、見えない一人の位置だけが群像を動かす構成へ変えた。" },
    { recipe: "空席の合唱", dist: "alien",
      text: "誰も座っていない椅子だけにマイクがあり、床の振動と会場の残響を増幅する。満席になるほど音楽が小さくなる劇場を作る。" },
    { recipe: "影の予告", dist: "alien",
      text: "人物の影が本体より一場面早く登場し、次に起きる動作を無言で予告する。影が一度だけ間違えたとき、本体が従うか拒むかは未決定。" },
    { recipe: "環境との契約", dist: "alien",
      text: "屋外公演で風が強い日は空中技が増え、雨の日は床の技が解禁される。天候を妨害でなく演目表の選択者にする。",
      sources: ["show_aital_a_ciel_ouvert", "show_baro_les_escapades"],
      inspiration: "屋外の共有環境と公共空間を上演条件として受け入れる考えを、当日の天候が演目順を決める契約へ具体化した。" },
    { recipe: "説明の先出し", dist: "alien",
      text: "舞台上には作品名と解説札だけが並び、対応する人物や物は後から一つずつ現れる。最後の札には、まだ存在しない演目名が書かれている。",
      sources: ["show_musical_wicked", "show_musical_phantom_of_the_opera"],
      inspiration: "結末や過去の物を先に置いて観客の読みを作る枠を、出来事より先に解説だけが存在する展示形式へ移した。" },
    { recipe: "境界で翻訳する", dist: "alien",
      text: "舞台中央の一本の線を越えるたび、同じ動作がダンス、手話、作業、競技へ意味を変える。身体は同じまま、観客の読みだけを移動させる。",
      sources: ["show_musical_lion_king", "show_immersive_sleep_no_more"],
      inspiration: "見えている仕掛けと像を同時に読む二重性と、場所ごとの観客ルールを、一本の境界で動作の意味だけが変わる仕組みにした。" },
    { recipe: "温度の舞台", dist: "alien",
      text: "演者の移動に合わせて床の一部が暖かく、別の場所が冷たくなる。観客には見えない温度差を、足裏と呼吸の変化だけで見せる。" },
    { recipe: "成長する上演", dist: "alien",
      text: "毎公演、一粒の種を同じ舞台へ植える。芽が出るまで物語は始まらず、成長後は植物の高さが技の順番を決める。",
      sources: ["show_casino2_wynn_tree_prosperity_2006", "show_baro_les_escapades"],
      inspiration: "時間や季節を樹木の変化に託す構造と、場所へ徴を増やす進行を、実際の生長が上演規則になる長期作品へ延長した。" },
    { recipe: "出来事を見せない", dist: "alien",
      text: "観客は室内で、外で進むショーの影、音、濡れた衣装だけを見る。終演後に外へ出ても、出来事の痕跡しか残っていない。",
      sources: ["show_immersive_sleep_no_more", "show_jp_theme2_tokyo_night_light"],
      inspiration: "観客が場所を移りながら断片を組み立てる体験と、建築面を夜の像に変える方法を、外の出来事を室内の痕跡だけで読む構成へ反転した。" },
    { recipe: "布の学習", dist: "alien",
      text: "大きな幕が群像の動きを一つずつ覚え、後半は人が動かず布だけが同じ振付を繰り返す。誰が操っているかは最後まで見せる。" },
    { recipe: "記録の偏り", dist: "alien",
      text: "成功した技の映像だけが直後に消去され、失敗だけがスクリーンへ蓄積する。終幕では失敗の記録を見ながら、同じ技を無音で一度だけ行う。" },
    { recipe: "建物を出演者にする", dist: "alien",
      text: "エレベーターの到着音、空調、非常灯、閉まる扉を出演順として扱う。人間は建物の合図が出た場所でしか演じられず、一度だけ合図に逆らう。",
      sources: ["show_jp_theme2_tokyo_night_light", "show_casino_wynn_macau_performance_lake_2006"],
      inspiration: "建築そのものを夜の舞台面にする方法と、施設に組み込まれた短い反復ショーの構造を、既存設備が出演順を出す上演へ移した。" },
  ],
