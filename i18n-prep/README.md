...
4. **レイアウト伸縮。** フランス語は日本語比で1.5〜2倍に伸びるボタンがある（例: 書き出す→Exporter は同等だが、
   ...
   fr で目視確認する工程を1つ入れる。
5. **版上げ。** 実装当日は舞台スケッチ側の正本 `stage.html` の `?v=`、
   `stage-sw.js` の `CACHE_NAME`、`build_stage.py` 再実行（派生Viewer更新と分離検証）、版ピンテスト。
...
> 追記 2026-09-09: 中国語UI（簡体・繁体）の実装発注書は `docs/I18N_ZH_WORKORDER_2026-09-09.md`（R1=WO-B3 → R2 パック化 → R3 中国語投入 → R4 版上げ・独立レビュー）。
