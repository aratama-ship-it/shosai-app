...
(function () {
  ...
  function drawFallRange(result) {
    ...
    fall.overlapPolygons.forEach((polygon) => {
      ...
      ctx.beginPath();
      pathPolygon(polygon);
      ctx.fillStyle = "rgba(238,55,48,0.72)";
      ctx.fill();
      ctx.restore();
    });
  ...
  if (els.probeReach) {
    els.probeReach.addEventListener("input", () => setProbeReach(els.probeReach.value));
    els.probeReach.addEventListener("change", () => setProbeReach(els.probeReach.value));
  }
