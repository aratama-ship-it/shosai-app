...
- 資料を使う編集計画では、出典・直接観察・解釈・舞台案を任意の根拠パックとして分離して残す
...
`stage_sketch_plan_edit` の `evidenceContext` は、1つの編集計画を1つの舞台案として、
次の4層を混ぜずに保存します。

- `sources`: ローカルPDF・画像、公式Web、書籍、調査記録、本人の入力
- `observations`: ページや画像から直接確認できたこと。確信できない場合は `uncertain`
- `interpretations`: 観察事実から導いた読み。必ず参照した `observationIds` を持つ
- `proposal`: この編集計画が舞台で何を試す案なのか。必ず参照した `interpretationIds` を持つ

根拠パックは `plans/` と、適用後の読み込み用JSONにある `editSummary` へ残ります。
舞台スケッチの正本 `projects/` には混ぜません。案は従来どおり、差分を本人が確認して
`confirmed: true` を渡すまで適用されません。複数案を比較するときは、最大3つを目安に
別々の編集計画として作ります。
