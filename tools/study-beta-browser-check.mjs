...
const base = process.env.STUDY_BASE || 'http://127.0.0.1:8802';
const output = new URL('../docs/study-links/qa/beta-casual/', import.meta.url);
...
try {
  ...
  assert.equal(await page.locator('#study-login').isVisible(),false);
  assert.match(await page.locator('[data-text=personal]').textContent(),/同期されません/);
  assert.equal(await page.locator('#study-name-field').isVisible(),true);
  checks.push('Beta invitation opens directly without sign-in and explains device-only notes');
  ...
  await click('#study-pen');
  for (const view of ['front','plan']) {
    await page.locator('#study-frame-host').evaluate(el => el.scrollIntoView({block:'center'}));
    const layer = frame().locator(`[data-pen-view="${view}"]`);
    let r=await layer.boundingBox(); await page.evaluate(y=>scrollTo(0,y),await page.evaluate(()=>scrollY)+r.y-100);
    await page.evaluate(()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)))); r=await layer.boundingBox();
  ...
  await click('#study-next'); await page.waitForFunction(()=>document.querySelector('#study-scenes').value==='qa-scene-1'); assert.equal(await page.locator('#study-note').inputValue(),'');
  await page.locator('#study-note').fill('場面2の個人メモ'); await click('#study-prev');
  ...
  await click('#study-send'); await page.waitForFunction(() => document.querySelector('#study-note-status').textContent.includes('共有しました'));
  ...
  await page.route('**/study/api/view/*/notes',route=>route.abort()); await click('#study-send');
  ...
  await page.locator('#study-note').fill(''); await click('#study-send');
  ...
  await page.waitForFunction(()=>document.querySelector('#study-status').textContent.includes('unavailable'),{timeout:15000});
  assert.equal(await page.locator('#study-workspace').isVisible(),false);
  assert.equal((await context.request.post(viewApi+'/notes',{headers:{Origin:base},data:{name:'A',text:'blocked',sceneId:document.project.scenes.find(s=>s.kind==='scene').id,revision:2}})).status(),404);
  await page.goto(base+'/study?lang=ja');
  await page.waitForFunction(()=>document.querySelector('#study-status').textContent.includes('招待リンク'));
  assert.equal(await page.locator('#study-login').isVisible(),false);
  checks.push('Online revocation hides the stage and blocks posts; no-token entry asks for an invitation');
  ...
  console.log(await page.evaluate(()=>({status:document.querySelector('#study-status')?.textContent,scene:document.querySelector('#study-scenes')?.value,note:document.querySelector('#study-note')?.value})),errors);
  throw error;
}finally{
