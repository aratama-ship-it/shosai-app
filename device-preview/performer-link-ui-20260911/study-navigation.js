...
(() => {
  ...
  window.SHOSAI_STUDY_NAVIGATION = ({ engine, canvases, cancelAnnotations }) => {
    ...
    function handleEvent(event) {
      // Narrow native-control allowlist; the editor event firewall remains in force.
      ...
      if (event.type === 'pointerdown' && v && event.button === 0) {
        ...
        if (pair.length === 2) {
          pair.forEach(cancelAnnotations); suppressed=true; begin(v,pair); return true;
        }
    ...
    return {
      ...
      // Capture the same visible viewport, including the transformed annotation layer.
      crop(view, source) {
