...
(function () {
  ...
  // 実在劇場は配布時の選択肢に含めない。旧ショーのID参照と、本人が取り込んだ
  // 会場ライブラリは使えるように残す（読み込み済みデータにはこの除外を掛けない）。
  window.SHOSAI_VENUES = {
    get list() { return VENUES.filter((venue) => !venue.realVenue).concat(readLibrary().map(customLegacyVenue)); },
    ...
    v2: {
      get list() { return VENUES_V2.filter((venue) => !venue.realVenue).concat(readLibrary()); },
      byId: venueV2ById,
    },
    library: {
