(() => {
  ...
  const dictionary = {
    ...
    productAbout: ['舞台スケッチ製品版について', 'About the full Stage Sketch product'],
    productAboutLabel: ['舞台スケッチ製品版について（別タブで開く）', 'About the full Stage Sketch product (opens in a new tab)'],
  ...
  function relabel() {
    ...
    $('study-product-link').href = 'https://stagesketch-try.juggler-arata.workers.dev' + (lang === 'en' ? '/en/beta' : '/beta');
  ...
  async function load() {
    ...
    try {
      ...
      frame = document.createElement('iframe'); frame.title = t('both'); frame.setAttribute('sandbox', 'allow-scripts'); frame.referrerPolicy = 'no-referrer'; frame.src = '/study-frame.html?v=6';
